...
class PostModal(discord.ui.Modal):
    def __init__(self, is_anon: bool, reply_target: str = None):
        title_text = "📝 匿名投稿" if is_anon else "📝 非匿名投稿"
        if reply_target:
            title_text += f" ({reply_target}へ返信)"
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not board_channel:
            await interaction.followup.send("エラー: BOARD_CHANNEL_ID が不正です。", ephemeral=True)
            return
        ...
        if log_channel:
            ...
            log_embed.add_field(name="表示タイプ", value="匿名" if self.is_anon else "非匿名")
